      ...
      <div class="input-group mb-3" style="width: 100%; margin-top:40px">
              ...
              <c:choose>
						...
						<c:otherwise>
							<c:forEach var="item" items="${list}">
								<tr onclick="location.href='/contact/detail.do?no=${item.no}'">
									<td scope="row">${row}</td>
									<td>${item.name}</td>
									<td>${item.email}</td>
									<td>${item.indate}</td>
        ...
        <nav aria-label="Page navigation example" 
          ...
          <c:forEach var="page" begin="${pi.startPage}" end="${pi.endPage}">
            <li class="page-item" style="margin-right: 0px">
            	<a class="page-link" href="/contact/list.do?cpage=${page}">${page}</a></li>
          </c:forEach>
          ...
          <c:choose>
            ...
            <li class="page-item" style="margin-right: 0px">
          	...
          	<c:otherwise>
          		<li class="page-item" style="margin-right: 0px">
              <a class="page-link" href="/contact/list.do?cpage=${pi.cpage+1}" 
